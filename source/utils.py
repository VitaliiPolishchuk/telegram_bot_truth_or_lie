...
def generate_list_enrolled_users(list_enrolled_user_data):
    ...
    f.write("count\tid\tfirst_name\tlast_name\n")
    for i,enrolled_user_data in enumerate(list_enrolled_user_data):
        f.write("{}\t{}\t{}\t{}\n".format(i + 1,enrolled_user_data.get_id(), enrolled_user_data.get_first_name(), enrolled_user_data.get_last_name()))
    f.close()

def generate_list_finished_users(list_finished_user_data):
    f = open(config.finished_user_file,"w")
    f.write("count\tid\tfirst_name\tlast_name\tpoints\tspended_time\n")
    for i,finished_user_data in enumerate(list_finished_user_data):
        f.write("{}\t{}\t{}\t{}\t{}\t{}\n".format(i + 1,finished_user_data.get_id(), finished_user_data.get_first_name(), finished_user_data.get_last_name(), 
...
def generate_list_top_users(list_finished_user_data):
    ...
    f.write("count\tid\tfirst_name\tlast_name\tpoints\tspended_time\n")
    for i,finished_user_data in enumerate(list_finished_user_data):
        f.write("{}\t{}\t{}\t{}\t{}\t{}\n".format(i + 1,list_finished_user_data[i].get_id(), list_finished_user_data[i].get_first_name(), list_finished_user_data[i].get_last_name(), 
